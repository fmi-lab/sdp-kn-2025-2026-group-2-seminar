...
- std::optional\<V> closest_key(const K& key) const - ако дървото не е празно, намира стойността в този възел от дървото, който има най-близката стойност на ключа до `key`;
- R accumulate(const K& low, const K& high, const std::function\<R(const V&, const R&)>& accumulator, const R& null_value) const - натрупва резултата от прилагането на функцията `accumulator` над всички елементи в дървото с ключ по-голям от `low` и по-малък от `high`. Натрупването да следва стратегията "ляво-корен-дясно";
- std::pair\<AVLTree\<K, V>, AVLTree\<K, V>> split(const K& key) const - разделя дървото на две дървета, като първото съдържа възлите с ключове по-малки от `key`, а второто съдържа възлите с ключове по-големи от `key`.
