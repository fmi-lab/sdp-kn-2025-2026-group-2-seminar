# Седмица 07 - Двоично дърво за търсене
...
- bool is_balanced() const - проверява дали дървото е балансирано. Казваме, че дърво е балансирано, ако разликата във височините на лявото и дясното му поддървета е по-малка или равна на 1 като и самите поддървета трябва също да са балансирани. Функцията трябва да има времева сложност от `O(V)`;
- std::optional\<V> kth_element(std::size_t k) const - намира стойността на k-тия елемент в дървото спрямо наредбата на ключовете;
...
- void narrow(const K& from, const K& to) - премахва всички върхове от дървото, които не са с ключове по-големи от `from` и по-малки от `to`.
