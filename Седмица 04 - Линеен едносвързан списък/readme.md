# Седмица 04 - Линеен едносвързан списък
...
- LinkedList\<T> intersection(const LinkedList& other) - намира сечението на два списъка (сечението не трябва да съдържа повторения);
- LinkedList\<T> merge(const LinkedList& other) - слива два сортирани списъка, резултатът е нов сортиран списък;
...
- const T& min_by(const std::function<bool(const T&, const T&)>& comparator) const - намира минималния елемент в списъка по подадена функция за сравнение на елементите;
...
## Задача 04 - Итeратор за обхождане в обратен ред
Към реализацията на линейния едносвързан списък от задача 00, добавете и имплементация на итератор, който обхожда елементите на списъка в обратен ред. Добавете следните допълнителни методи към списъка:

- ReverseIterator rbegin() const - 0
- ReverseIterator rend() const - aналог на `end`, но за ReverseIterator

Обхождането на елементите трябва да бъде оптимално, т.е. достъпът до следващия елемент трябва да има времева сложност от O(1).

### Упътване:
Помислете коя структура от данни може да използвате, за да постигнете оптималност.