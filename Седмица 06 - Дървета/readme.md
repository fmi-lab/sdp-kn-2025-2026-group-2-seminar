...
**Важно**: това е абстрактен модел на дърво с произволен брой наследници. В практиката едва ли ще реализирате дървото по този начин - най-вероятно ще имате повече данни по върховете и структурата ще се получи по-скоро естествено и няма да се наложи да правите отделна структура за неговото представяне.
...
- bool leaf() const - проверява дали дървото няма наследници;
...
- std::vector\<std::vector\<T>> paths() const - намира всички пътища от корена на дървото до всяко негово листо;
- std::optional\<T> lowest_common_ancestor(const T& lhs, const T& rhs) const - намира най-близкия общ родител на върховете в дървото, съдържащи подадените стойности;
- bool is_mirror() const - проверява дали дървото е огледално. Огледално дърво наричаме такова дърво, на което дясното поддърво има обратно симетрична структура спрямо лявото.

### Примерно огледално дърво
```c++
/*
         4
       /  \         
